...
export async function getCategoryProducts({
  ...
  for (const productIds of chunkedProductData) {
    const newVariantIds = await getCategoryProductVariantIds(productIds, requireThumbnail);
  ...
  return products.filter((p) => Boolean(p.variants?.length)) as unknown as Product[];
}
